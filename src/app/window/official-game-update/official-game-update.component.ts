...
export class OfficialGameUpdateComponent implements OnInit, OnDestroy {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                var r = request(options, (err, response, body) => {
                    console.log(url, ' ok', response.statusCode, err)
                    if (err) {
                        reject(err);
                    }
                ...
                progress(r).on("progress", (state) => {
                    let progress = state.percent * weight;
                    this.addProgress(progress - currentProgress);
                    currentProgress = progress;
                })
                .on('error', function (e) {
                    console.log('download3 error : ', e)
                })
                .on('end', function () {
                    console.log('end !')
                });
            } catch (e) {
                console.log('download1 error', e)
                reject(e);
            }
        });
    }
    ...
    async downloadMissingFiles(manifest, differences, basePath = "") {
        let files = {};
        try {
            console.log('downloadMissingFiles', differences)
            for (var i in differences) {
                if (differences[i] == 1) {
                    files[i] = await this.download(basePath + manifest.files[i].filename, false, 
                    this.getFileEstimatedWeight(manifest.files[i].filename));
                }
            }
            console.log('downloadMissingFiles out')
    ...
    downloadAndSaveFile(url, filePath) {
        return new Promise((resolve, reject) => {
            try {
                ...
                Logger.error(e.message); reject(e.message); }
        });
    }
